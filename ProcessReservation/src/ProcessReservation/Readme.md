...
WORKDIR /src
COPY ["ProcessReservation.fsproj", "ProcessReservation/"]
RUN dotnet restore "ProcessReservation/ProcessReservation.fsproj"

WORKDIR "/src/ProcessReservation"
COPY . .
RUN dotnet build "ProcessReservation.fsproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "ProcessReservation.fsproj" \
...
Execute unit tests
```
cd "ProcessReservation/test/ProcessReservation.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "ProcessReservation/src/ProcessReservation"
